/* 
    ...
    flex minir.l
    bison minir.y
    g++ -std=c++11 reinbolth.tab.c -o parser
...
%type <typeInfo> N_ASSIGNMENT_EXPR N_ARGS N_THEN_EXPR N_COND_IF
...
N_IF_EXPR       : N_COND_IF T_RPAREN N_THEN_EXPR 
                ...
                | N_COND_IF T_RPAREN N_THEN_EXPR T_ELSE N_EXPR
                {
                    ...
                    if(($5.type == FUNCTION) || ($3.type == FUNCTION))
                    {
                        semanticError(3, ERR_CANNOT_BE_FUNCT);
                    }
...
N_ASSIGNMENT_EXPR : T_IDENT N_INDEX
			    ...
			    */
                    ...
                    if ((exprTypeInfo.type != LIST) && ($2 == INDEX_PROD))
                    {
                    ...
                    if(exprTypeInfo.isParam == false)
                    {
                        scopeStack.top().changeEntry(
                            SYMBOL_TABLE_ENTRY(lexeme,
                            {$5.type, $5.numParams, $5.returnType, true})
                        );
                    }
		     ...
		     T_RPAREN N_COMPOUND_EXPR
                {
                    ...
                    {
                        $$.type = FUNCTION;
                        $$.numParams = scopeStack.top().countEntry();
                        $$.returnType = $6.type;
                    }
			    endScope();
                }
...
N_ARG_LIST      : {numExprs = 0;} N_ARGS
                {
                    printRule("ARG_LIST", "ARGS");
                }
                | {numExprs = 0;} N_NO_ARGS
                {
