...
typedef struct {
  int type;
  int intVal;           // only applicable if INT
  float floatVal;       // only applicable if FLOAT
  bool boolVal;         // only applicable if BOOL
  string* stringVal;     // only applicable if str
}LISTENTRY;
...
typedef struct {
  ...
  bool boolVal;         // only applicable if BOOL
  string* stringVal;     // only applicable if STR
  vector<LISTENTRY> *listVal;   // only for LIST
} TYPE_INFO;
...
public:
                     ...
                     const TYPE_INFO theType)
  {
    ...
    typeInfo.listVal = theType.listVal;
  }
